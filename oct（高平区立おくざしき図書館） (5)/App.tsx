...
import { Book, ModalState, NewsItem, Notice, ClosedDate, MonthlyFeature, SurveyQuestion, Reservation } from './types';
...
import { FileText, ArrowRight, Calendar as CalendarIcon, BookOpen, Bell, Search, Info, Newspaper, ChevronRight } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      unsubBooks(); unsubNews(); unsubNotices(); unsubDates(); unsubFeature(); unsubReservations();
    };
  }, []);
  ...
  // 特定の本が予約されているかチェック (StatusがCOMPLETED以外)
    ...
    <div className="min-h-screen bg-[#fdfdfd] text-oct-950 font-serif">
      ...
      <section className="relative h-[80vh] flex items-center justify-center overflow-hidden bg-oct-950">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center text-white">
          ...
          <div className="flex justify-center gap-4 animate-slide-up">
            ...
            <button onClick={() => openModal('ALL_BOOKS')} className="bg-white/10 backdrop-blur-md border border-white/30 text-white px-8 py-4 rounded-full font-bold hover:bg-white/20 transition-all">蔵書検索</button>
          </div>
        </div>
      </section>
      ...
      <main className="max-w-7xl mx-auto px-4 py-20 grid grid-cols-1 lg:grid-cols-12 gap-16">
        <div className="lg:col-span-8 space-y-24">
          ...
          <section>
            <div className="flex items-center justify-between mb-8 border-b pb-4">
              <h2 className="text-3xl font-bold serif flex items-center gap-3"><Newspaper className="text-oct-600" /> 図書館だより</h2>
            </div>
            <div className="grid md:grid-cols-2 gap-6">
              {news.slice(0, 2).map(item => (
                <div key={item.id} onClick={() => openModal('NEWS_DETAIL', item)} className="group bg-white border border-oct-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer">
                  <div className="aspect-video relative overflow-hidden bg-oct-50">
                    <img src={item.previewImageUrl || 'https://images.unsplash.com/photo-1506880018603-83d5b814b5a6?q=80&w=1200'} className="w-full h-full object-cover group-hover:scale-105 transition-transform" alt="Newsletter" />
                    <div className="absolute inset-0 bg-black/20 group-hover:bg-black/0 transition-colors" />
                  </div>
                  <div className="p-6">
                    <span className="text-xs text-oct-400 font-bold mb-2 block">{item.date}</span>
                    <h3 className="font-bold text-lg mb-2">{item.title}</h3>
                    <p className="text-xs text-gray-400">詳細・ダウンロードはこちら</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Recommended Books - TEXT ONLY NO THUMBNAILS */}
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {books.filter(b => b.isRecommended).map(book => (
                <div key={book.id} onClick={() => openModal('BOOK_DETAIL', book)} className="group cursor-pointer bg-white p-6 rounded-2xl border border-oct-100 shadow-sm hover:shadow-md hover:border-oct-300 transition-all">
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-[10px] font-bold text-oct-500 bg-oct-50 px-2 py-1 rounded">{book.category}</span>
                    {isBookReserved(book.id) && (
                      <span className="text-[10px] font-bold text-red-500 border border-red-100 px-2 py-1 rounded">貸出中</span>
                    )}
                  </div>
                  <h4 className="font-bold text-xl mb-1 group-hover:text-oct-700 transition-colors">{book.title}</h4>
                  <p className="text-sm text-gray-500 mb-4">{book.author}</p>
                  <div className="flex justify-end">
                    <span className="text-xs text-oct-400 flex items-center gap-1 group-hover:underline">詳細を見る <ChevronRight size={14} /></span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Modal isOpen={modalState.type !== 'NONE'} onClose={closeModal} onNavigate={openModal}>
        ...
        {modalState.type === 'ALL_BOOKS' && <AllBooksModal books={books} onBookClick={(b) => openModal('BOOK_DETAIL', b)} />}
      </Modal>
    </div>
  );
};
